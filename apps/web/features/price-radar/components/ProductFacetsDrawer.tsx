        ...
        <header className="flex min-h-16 items-center justify-between gap-3 border-b border-inest-line px-5">
          ...
          <div className="flex items-center gap-1">
            <button
              type="button"
              onClick={onClear}
              className="min-h-11 px-3 text-sm font-black text-inest-blue hover:text-blue-700"
            >
              Limpar tudo
            </button>
            <button
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-6 scrollbar-stable">
          ...
          {models ? (
            <FacetGroup
              ...
              onToggleShowAll={() => setShowAllModels((current) => !current)}
            />
          ) : null}
          ...
          {additionalGroups.map((group) => (
            <FacetGroup key={group.title} group={group} />
          ))}
  ...
  return (
    <FacetSection title={group.title}>
      {group.presentation === 'colors' ? (
          ...
          {visibleOptions.map((option) => {
            ...
            return (
              <button
                ...
                className={`grid min-h-[76px] place-items-center gap-1 rounded-lg border p-2 text-center text-xs font-bold transition-colors ${selected ? 'border-inest-blue bg-blue-50 text-inest-blue' : 'border-inest-line text-inest-muted hover:bg-inest-soft'}`}
              >
                <span
                  className="h-7 w-7 rounded-full border border-slate-300 shadow-sm"
                  style={{ backgroundColor: option.swatch ?? colorSwatch(option.value) }}
                />
        ...
        <div className={group.columns ? 'grid grid-cols-2 gap-x-4' : 'grid'}>
          {visibleOptions.map((option) => (
            <label
              key={option.value}
              className="flex min-h-11 cursor-pointer items-center gap-3 text-sm font-bold text-inest-text"
            >
              <input
                type="checkbox"
      ...
      {!visibleOptions.length ? (
        <p className="text-sm text-inest-muted">Nenhuma opcao disponivel.</p>
      ) : null}
      {group.collapsible && group.options.length > 8 ? (
        <button
          type="button"
          onClick={onToggleShowAll}
          className="min-h-11 text-sm font-black text-inest-blue"
        >
...
function ConditionGroup({
  ...
  value,
  onChange,
}: NonNullable<ProductFacetsDrawerProps['condition']>) {
  return (
...
function PriceGroup({
  ...
  onMinChange,
  onMaxChange,
}: NonNullable<ProductFacetsDrawerProps['price']>) {
  ...
  return (
    <FacetSection title={currencyLabel ? `Faixa de preço (${currencyLabel})` : 'Faixa de preço'}>
      <div className="grid gap-3">
        <input
          ...
          onChange={(event) =>
            onMinChange(String(Math.min(Number(event.target.value), selectedMax)))
          }
        ...
        <input
          ...
          onChange={(event) =>
            onMaxChange(String(Math.max(Number(event.target.value), selectedMin)))
          }
          className="w-full accent-inest-blue"
        />
...
function FacetCount({ count }: { count: number }) {
  return (
    <span className="rounded-md bg-inest-soft px-2 py-1 text-[10px] font-black text-inest-muted">
      {count}
    </span>
  );
}
...
function colorSwatch(value: string) {
  ...
  if (normalized.includes('cinza') || normalized.includes('gray') || normalized.includes('grey'))
    return '#64748b';
